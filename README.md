...
<head>
  ...
  <title>Dinge gegen Langeweile — Deluxe</title>

  <!-- GSAP (für flüssige Animationen) -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js" defer></script>

  <style>
    /* ---------------- Reset & Farben (CSS-Variablen) ---------------- */
    :root{
      --bg-100: #f6f8fb;
      --bg-900: #07101a;
      ...
      --accent2: #4fd1c5;
      --text: #072033;
      --muted: rgba(7,32,51,0.65);
      --glass-2: rgba(8,17,27,0.45);
      --duration: 380ms;
    }
    /* Dark theme variables will be set via JS to animate */
    html,body{height:100%;margin:0;padding:0;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial}
    *{box-sizing:border-box}
    body{
      background: linear-gradient(180deg,var(--bgA, #0f1724), var(--bgB, #0b1220));
      color:var(--textA, #e6eef8);
      overflow-x:hidden;
    ...
    .menu-btn:focus,.settings-btn:focus{outline:2px solid rgba(255,255,255,0.06)}
    .logo{
      display:flex;gap:10px;align-items:center;font-weight:800;letter-spacing:0.3px;
    }
    ...
    .greeting{font-size:14px;color:rgba(230,238,248,0.85);font-weight:700}

    nav.left-panel{
      position:fixed;left:18px;top:74px;z-index:9;
    ...
    /* ---------- Top right settings panel (slide out) ---------- */
    .settings-toggle{position:fixed;right:18px;top:18px;z-index:12}
    .settings-drawer{
      position:fixed;right:18px;top:64px;width:360px;max-width:92vw;z-index:12;
      background:linear-gradient(180deg, rgba(3,6,23,0.65), rgba(3,6,23,0.55));
    ...
    .section-body{padding:8px 6px 12px 6px;display:none} /* accordion hidden default */
    .toggle{display:inline-flex;align-items:center;gap:6px}

    /* nice inputs */
    input[type="text"], select, textarea{
      width:100%;padding:10px;border-radius:10px;border:1px solid rgba(255,255,255,0.06);
      background:transparent;color:inherit;font-weight:600;margin-top:8px;
    }
    .row{display:flex;gap:8px;align-items:center}
    .small{font-size:13px;color:rgba(230,238,248,0.75)}

    /* big generator card & grid */
    ...
    .card{
      ...
      border-radius:14px;padding:18px;margin-bottom:14px;backdrop-filter:blur(6px);
      box-shadow:0 8px 30px rgba(3,6,23,0.55);
    }
    .big-btn{
      display:inline-flex;align-items:center;gap:10px;padding:12px 16px;border-radius:12px;border:none;
      cursor:pointer;font-weight:800;background:linear-gradient(90deg,var(--accent),var(--accent2));
      color:#031124;box-shadow:0 10px 30px rgba(124,92,255,0.14);
    }
    ...
    /* fancy switch */
    .switch{
      width:56px;height:30px;border-radius:999px;background:linear-gradient(90deg,#222,#333);position:relative;padding:5px;cursor:pointer;
    }
    ...
    /* micro styles for editable list */
...
<body>
  ...
  <!-- Canvas Partikel (wie vorher) -->
  <canvas id="canvas-bg"></canvas>

  <div class="app">
    <header>
      <div class="left-controls">
        <button class="menu-btn" id="btn-open-menu" title="Öffne Seitenmenü">☰</button>
        <div class="logo">
          <div class="logo-badge">DG</div>
          <div>
            <div style="font-size:15px">Dinge gegen</div>
            <div class="greeting" id="greet">Langeweile — Projekt</div>
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;gap:10px;align-items:center">
        ...
        <div class="settings-toggle">
          <button class="settings-btn" id="settings-btn" title="Einstellungen">⚙️</button>
        </div>
      </div>
    </header>
    ...
    <nav class="left-panel" id="leftPanel" style="display:none">
      <a href="#" data-page="generator" class="active">🎲 Generator</a>
      <a href="#" data-page="popular">👥 Was andere machen</a>
      <a href="#" data-page="about">ℹ️ Über</a>
    </nav>
    ...
    <main>
      <section class="hero">
        ...
        <div class="left-card">
          <div class="card" id="genCard">
            <h2 id="hero-title">Sofort Ideen gegen Langeweile ✨</h2>
            <p class="muted" id="sub-title">Klicke auf "Los!" oder wähle Persönlichkeit für angepasste Vorschläge.</p>

            <div style="display:flex;gap:12px;align-items:center;margin-top:12px">
              <button class="big-btn" id="btn-gen">Los! 🎲</button>
              <div style="flex:1">
            ...
            <div style="margin-top:14px" id="idea-grid-wrap">
              <div style="font-weight:800;margin-bottom:8px">Vorschläge</div>
        ...
        <!-- Right column: Top picks, settings preview -->
        <aside class="right-column">
          <div class="card">
            <div style="display:flex;justify-content:space-between;align-items:center">
              <div style="font-weight:800">Top Picks</div>
              <div class="muted">angepasst an Persönlichkeit</div>
            </div>
            <div class="grid" id="top-picks" style="margin-top:10px"></div>
          </div>

          <div class="card" style="margin-top:12px">
            <div style="font-weight:800;margin-bottom:8px">Beliebte Aktivitäten</div>
      ...
      <section id="page-popular" style="display:none;margin-top:14px">
        <div class="card">
          <h3>Was Andere machen — Beispiele</h3>
          <p class="muted">Eine kleine Sammlung von Aktivitäten, die oft gewählt werden.</p>
      ...
      <section id="page-about" style="display:none;margin-top:14px">
        <div class="card">
          ...
          <p class="muted">Dieses Projekt ist für ein Schulprojekt — gebaut mit HTML/CSS/JS und krassen Animationen.</p>
        </div>
      </section>

      <footer>
        © <span id="year"></span> Dinge gegen Langeweile — Erstellt von dir. Viel Erfolg, Bro! 🚀
      </footer>
    </main>
  </div>

  <!-- SETTINGS DRAWER (initially hidden) -->
  <div id="settingsDrawer" class="settings-drawer" style="display:none">
    <div class="settings-header">
      <div style="font-weight:800">Einstellungen</div>
      <div class="small">Speichert lokal ✅</div>
    </div>
    ...
    <div class="settings-section">
      ...
      <div class="section-body" id="theme-body">
        <p class="small">Krasse Animation beim Wechsel — einfach den Schalter klicken.</p>
    ...
    <div class="settings-section">
      <div class="section-title" data-accordion="name">
        <div><strong>Name & Begrüßung</strong><div class="small">Wird lokal gespeichert</div></div>
      ...
      <div class="section-body" id="name-body">
        ...
        <div style="display:flex;gap:8px;margin-top:8px">
          ...
          <button class="big-btn" id="clear-name" style="background:transparent;color:var(--accent);box-shadow:none;border:1px solid rgba(255,255,255,0.04)">Zurücksetzen</button>
        </div>
      </div>
    </div>
    ...
    <div class="settings-section">
      <div class="section-title" data-accordion="personality">
        <div><strong>Persönlichkeit</strong><div class="small">Verändert Vorschläge</div></div>
        <div class="toggle">▾</div>
      </div>
      <div class="section-body" id="personality-body">
        <label class="small">Preset auswählen</label>
        <select id="person-select" style="margin-top:8px"></select>

        <div style="margin-top:10px">
          <label class="small">Eigene Persönlichkeit hinzufügen (Name + Tags, kommagetrennt)</label>
          ...
          <div style="display:flex;gap:8px;margin-top:8px">
            <button class="big-btn" id="add-custom-person">Hinzufügen</button>
            <button class="big-btn" id="del-custom-person" style="background:transparent;border:1px solid rgba(255,255,255,0.04);color:var(--accent)">Löschen</button>
          </div>
        </div>

        <div style="margin-top:10px">
          <div style="font-weight:700;margin-bottom:6px">Deine Persönlichkeiten</div>
  ...
  // ---- Canvas background (lightweight partikel) ----
  ...
  const particles = []; const PCOUNT = Math.max(32, Math.floor((innerWidth * innerHeight) / 28000));
  ...
  const ideas = [
    ...
    {title:'Fake-Interview', tag:'Spiel', text:'Stell dir berühmte Personen vor und führe ein Interview.'},
    {title:'Sprachen-Blitz', tag:'Lernen', text:'Lerne 10 neue Wörter in einer Sprache.'},
    {title:'Musik-Mix', tag:'Musik', text:'Erstelle eine 6-Song Playlist für gute Laune.'},
    {title:'Podcast-Mikro', tag:'Tech', text:'Nimm eine 3-minütige Gedanken-Notiz auf.'}
  ];
  ...
  document.getElementById('btn-open-menu').addEventListener('click', ()=>{
    ...
    if(leftPanel.style.display === 'block'){
      gsap.from(leftPanel, {x:-20, opacity:0, duration:0.45, ease:'power3.out'});
    } else {
      gsap.to(leftPanel, {x:-12, opacity:0.6, duration:0.35});
    }
  });

  function openPage(p){
    // pages: generator, popular, about
    ...
    pageTitle.textContent = p==='generator' ? 'Generator' : (p==='popular' ? 'Was andere machen' : 'Über');
  }

  // ---- Accordion behavior for settings (open/close) ----
  document.querySelectorAll('.section-title').forEach(title=>{
    title.addEventListener('click', ()=>{
      const acc = title.dataset.accordion;
      ...
      if(isOpen){
        ...
        gsap.fromTo(body, {height:0, opacity:0}, {height:'auto', opacity:1, duration:0.32, ease:'power3.out'});
      }
    });
  });

  // ---- Theme variables & animations ----
  // We'll animate background color and accent using GSAP timeline for MEGA effect
  ...
  function applyThemeInstant(mode){
    if(mode === 'dark'){
      ...
      document.documentElement.style.setProperty('--textA', '#e6eef8');
      ...
      document.documentElement.style.setProperty('--textA', '#072033');
      document.documentElement.style.setProperty('--accent', '#5b21b6');
      document.documentElement.style.setProperty('--accent2', '#0ea5a5');
    }
  }
  // fancy animated flip/morph
  function animateThemeSwitch(to){
    const tl = gsap.timeline();
    if(to === 'dark'){
      tl.to('body', {duration:0.45, filter:'brightness(0.85) saturate(1.05) blur(0px)', ease:'power1.inOut'});
      tl.to({}, {duration:0.05}); // tiny pause
      tl.to(document.documentElement, {cssVars: {'--bgA': '#0f1724','--bgB':'#07101a','--textA':'#e6eef8','--accent':'#7C5CFF','--accent2':'#4fd1c5'}, duration:0.7, ease:'power2.inOut'}, "<");
      tl.fromTo('#canvas-bg', {opacity:0.7},{opacity:0.95,duration:0.9}, "<");
      tl.to('body', {duration:0.6, filter:'none', ease:'power2.out'});
    } else {
      tl.to('body', {duration:0.45, filter:'brightness(1.1) saturate(1.05)', ease:'power1.inOut'});
      tl.to(document.documentElement, {cssVars: {'--bgA': '#f6f8fb','--bgB':'#eef3fb','--textA':'#072033','--accent':'#5b21b6','--accent2':'#0ea5a5'}, duration:0.7, ease:'power2.inOut'}, "<");
      tl.fromTo('#canvas-bg', {opacity:1},{opacity:0.75,duration:0.9}, "<");
      tl.to('body', {duration:0.6, filter:'none', ease:'power2.out'});
    }
  }
  ...
  // extra buttons for FX (demo)
  document.getElementById('theme-fx1').addEventListener('click', ()=>{ animateThemeSwitch(themeState.mode === 'dark' ? 'light' : 'dark'); themeState.mode = themeState.mode === 'dark' ? 'light' : 'dark'; localStorage.setItem('dg_theme', themeState.mode); themeSwitch.classList.toggle('on'); });
  document.getElementById('theme-fx2').addEventListener('click', ()=>{ /* simpler fade */ gsap.to(document.documentElement, {duration:0.6, cssVars:{'--bgA': themeState.mode==='dark'?'#f6f8fb':'#0f1724'}, onComplete:()=>{ themeState.mode = themeState.mode==='dark' ? 'light' : 'dark'; localStorage.setItem('dg_theme', themeState.mode); themeSwitch.classList.toggle('on'); }}); });

  // ---- Settings drawer open/close with animated icon ----
  settingsBtn.addEventListener('click', ()=>{
    if(settingsDrawer.style.display === 'block'){
      ...
      gsap.fromTo(settingsDrawer, {scale:0.96, opacity:0, y:-6}, {scale:1, opacity:1, y:0, duration:0.36, ease:'back.out(1.1)'});
    }
  });
  ...
  function setGreeting(name){
    if(name && name.trim().length){
      greetEl.textContent = `Hey ${name.trim()} 👋`;
    } else {
      greetEl.textContent = 'Dinge gegen Langeweile — Projekt';
    }
  }
  ...
  saveNameBtn.addEventListener('click', ()=>{
    ...
    if(v.trim().length){
      localStorage.setItem('dg_name', v.trim());
      setGreeting(v);
      gsap.fromTo(greetEl, {y:-6, opacity:0}, {y:0, opacity:1, duration:0.5, ease:'elastic.out(1,0.6)'});
    }
  });
  clearNameBtn.addEventListener('click', ()=>{
    ...
    nameInput.value = '';
    setGreeting('');
    gsap.fromTo(greetEl, {scale:1.08, opacity:0.6}, {scale:1, opacity:1, duration:0.32});
  });

  // ---- Personalities (presets + custom) ----
  ...
  function refreshPersonSelect(){
    ...
    const optGroup1 = document.createElement('optgroup'); optGroup1.label = 'Presets';
    defaultPresets.forEach(p=>{ const o=document.createElement('option'); o.value = 'preset:'+p.key; o.textContent = p.name; optGroup1.appendChild(o); });
    personSelect.appendChild(optGroup1);
    if(customPersons.length){
      const optGroup2 = document.createElement('optgroup'); optGroup2.label='Eigene';
      customPersons.forEach((p, idx)=>{ const o=document.createElement('option'); o.value = 'custom:'+idx; o.textContent = p.name; optGroup2.appendChild(o); });
      personSelect.appendChild(optGroup2);
    }
  }

  function renderCustomList(){
    customList.innerHTML = '';
    customPersons.forEach((p, idx)=>{
      const chip = document.createElement('div'); chip.className='chip';
      chip.innerHTML = `<div style="font-weight:800">${p.name}</div><div style="opacity:.85">${p.tags.join(', ')}</div><button data-idx="${idx}">✕</button>`;
      chip.querySelector('button').addEventListener('click', ()=>{
        customPersons.splice(idx,1); localStorage.setItem('dg_custom_persons', JSON.stringify(customPersons)); renderCustomList(); refreshPersonSelect();
      });
  ...
  addCustomBtn.addEventListener('click', ()=>{
    const n = (customName.value||'').trim(); const t = (customTags.value||'').trim();
    if(!n) return alert('Bitte Namen angeben');
    const tags = t ? t.split(',').map(s=>s.trim()).filter(Boolean) : ['Alltag'];
    customPersons.push({name:n, tags:tags});
    localStorage.setItem('dg_custom_persons', JSON.stringify(customPersons));
    customName.value=''; customTags.value='';
    renderCustomList(); refreshPersonSelect();
    gsap.fromTo('#custom-list',{y:6,opacity:0},{y:0,opacity:1,duration:0.45, ease:'power3.out'});
  });

  delCustomBtn.addEventListener('click', ()=>{
    // quick delete last custom
    if(!customPersons.length) return alert('Keine eigenen Persönlichkeiten vorhanden');
    customPersons.pop(); localStorage.setItem('dg_custom_persons', JSON.stringify(customPersons)); renderCustomList(); refreshPersonSelect();
  });

  // initialize personalities UI
  refreshPersonSelect(); renderCustomList();

  // ---- Idea rendering & personality influence ----
  function scoreIdeaForPerson(idea, person){
    // simple scoring: +1 if idea.tag in preferred tags
    let score = 0;
    const t = idea.tag.toLowerCase();
  ...
  function getSelectedPerson(){
    ...
    if(val.startsWith('preset:')){
      ...
      return defaultPresets.find(p=>p.key===key) || defaultPresets[0];
    } else {
      const idx = parseInt(val.split(':')[1],10);
      return customPersons[idx] || defaultPresets[0];
    }
  }
  ...
  function renderIdeasGrid(){
    ...
    scored.forEach(s=>{
      ...
      el.innerHTML = `<div style="font-weight:800">${it.title}</div><div class="muted" style="margin-top:6px">${it.tag}</div><div style="margin-top:8px">${it.text}</div>`;
      el.addEventListener('click', ()=>{ showGenerated(`${it.title} — ${it.text}`); gsap.fromTo(el,{y:0},{y:-6,duration:0.18,yoyo:true,repeat:1}) });
      ideasGrid.appendChild(el);
    });
    // top picks
    topPicks.innerHTML = '';
    scored.slice(0,4).forEach(s=>{
      const it = s.i; const el = document.createElement('div'); el.className='idea'; el.style.minHeight='80px';
      el.innerHTML = `<div style="font-weight:800">${it.title}</div><div class="muted">${it.tag}</div>`;
  ...
  personSelect.addEventListener('change', ()=>{
    renderIdeasGrid();
    gsap.fromTo('#ideas-grid',{opacity:0,y:8},{opacity:1,y:0,duration:0.45});
  });

  renderIdeasGrid();

  // ---- Generator button ----
  function showGenerated(txt){
    genResult.innerHTML = txt;
    gsap.fromTo(genResult, {y:10, opacity:0}, {y:0, opacity:1, duration:0.46, ease:'power3.out'});
  }
  btnGen.addEventListener('click', ()=>{
    ...
    for(const item of scored){
      if(r < item.s){ showGenerated(`${item.i.title} — ${item.i.text}`); break; }
      r -= item.s;
    }
    gsap.fromTo(btnGen, {scale:1},{scale:0.96,duration:0.06,yoyo:true,repeat:1});
  });

  // ---- Popular page sample list ----
  ...
  function renderPopular(){
    ...
    samplePopular.forEach(p=>{
      const el = document.createElement('div'); el.className='idea';
      el.innerHTML = `<div style="font-weight:800">${p.title}</div><div class="muted">${p.by}</div><div style="margin-top:8px">${p.text}</div>`;
      popularList.appendChild(el);
    });
  }
  renderPopular();

  // ---- Left nav show on load for desktop (small tweak) ----
  if(window.innerWidth > 900) leftPanel.style.display = 'block';

  // ---- Close settings when clicking outside (small) ----
  ...
  // ---- Load stored theme on start smoothly ----
  window.addEventListener('load', ()=>{
    ...
    // apply stored selected person if any
    const storedPersonIdx = localStorage.getItem('dg_selected_person');
    if(storedPersonIdx){
      // try to set selection (could be custom/preset encoded)
      personSelect.value = storedPersonIdx;
    } else {
  ...
  // save selected person on change
  personSelect.addEventListener('change', ()=>{ localStorage.setItem('dg_selected_person', personSelect.value); });

  // save customPersons initially
  localStorage.setItem('dg_custom_persons', JSON.stringify(customPersons));

  // ---- Close behavior for accordions: open first by default ----
  document.querySelectorAll('.settings-section .section-body').forEach((b,i)=>{
    if(i===0){ b.style.display='block'; gsap.fromTo(b,{height:0,opacity:0},{height:'auto',opacity:1,duration:0.36}); }
  });

  // ---- small extras: clicking idea grid header animates cards ----
  document.getElementById('idea-grid-wrap').addEventListener('click', ()=>{
    gsap.fromTo('.idea',{y:6,opacity:0.6},{y:0,opacity:1,duration:0.5,stagger:0.04, ease:'power3.out'});
  });

  // ---- keyboard shortcut: S opens settings ----
  document.addEventListener('keydown', (e)=>{
    if(e.key.toLowerCase() === 's'){ settingsBtn.click(); }
    if(e.key === 'Escape'){ if(settingsDrawer.style.display==='block') { gsap.to(settingsDrawer, {scale:0.96, opacity:0, duration:0.22, onComplete:()=>settingsDrawer.style.display='none'}); } }
  });

  // ---- small helpers: persist theme on unload ----
  ...
  // ---- Small demo buttons for mini-games (not full games, placeholders) ----
  document.getElementById('open-tictactoe').addEventListener('click', ()=> alert('Tic-Tac-Toe Demo - später ausbauen 😎'));
  document.getElementById('open-memory').addEventListener('click', ()=> alert('Memory Demo - später ausbauen 😎'));
  document.getElementById('open-quickdraw').addEventListener('click', ()=> alert('Quick-Draw Demo - später ausbauen 😎'));

  // ---- Make sure personSelect gets initial value (if cleared) ----
